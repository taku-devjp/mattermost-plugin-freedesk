...
| 管理機能 | デスクの登録・編集・無効化（RHS 管理タブ）、予約代理取消 |
| 操作手段 | App Bar ボタン（画面右上）→ RHS パネル（予約タブ / 管理タブ） |
...
sequenceDiagram
    ...
    U->>M: App Bar ボタンで予約 UI を開く
    M->>P: 予約 API 呼び出し
    P->>P: 二重予約チェック
    P-->>M: 予約結果
    M-->>U: UI 更新 / チャンネル通知（設定時）
```
...
| ID | 要件 | 優先度 |
|----|------|--------|
| FR-030 | プラグイン管理者は RHS 内の**管理タブ**からデスク（名称・表示順・有効/無効）を CRUD できる。初版の管理対象は社員フリー1〜3、フリーデスク4〜7 の **7 席**（名称のみ異なり予約ルールは同一）。初回有効化（`OnActivate`）時に 7 席を自動登録する | Must |
...
| ID | 要件 | 優先度 |
|----|------|--------|
| FR-040 | 画面右上 App Bar のプラグインボタンから、RHS に予約 UI（マトリクス）を表示できる。通知先チャンネルの設定有無に依存しない | Must |
...
| FR-044 | マトリクス UI は列ヘッダー（デスク名）を固定し、横スクロール可能とする | Must |
| FR-045 | マトリクス UI の日付行は **1 か月単位** でページング表示する | Must |
| FR-046 | プラグイン管理者には RHS 内に**管理タブ**を表示する（一般ユーザーには非表示） | Must |
...
| RHS | Mattermost Right Hand Side パネル（右サイドパネル）。予約 UI の表示先 |
...
- App Bar ボタンを押すと RHS に予約 UI が開く
- 通知先チャンネルの設定有無とは独立して常に利用可能

### 10.2 予約マトリクス（RHS・予約タブ）
...
- 列ヘッダー（デスク名）を固定、**横スクロール**可能
- 日付行は **1 か月単位** でページング（前月 / 翌月で切り替え）
...
### 10.3 管理タブ（RHS・プラグイン管理者のみ）

- RHS 内の**管理タブ**からデスクの CRUD（名称・表示順・有効/無効）を行う
- 一般ユーザーには管理タブを表示しない
...
| フェーズ | 内容 |
|----------|------|
| v0.1.0（MVP） | 予約 CRUD、App Bar → RHS マトリクス UI、7 席自動登録、デスク管理（管理タブ）、チャンネル通知 |
| v0.2.0 | スラッシュコマンド |
