...
| FR-011 | 表示期間および予約可能期間は同一とし、「今日から約 2 か月先」まで（Mattermost System Admin がプラグイン設定で 1〜6 か月に変更可能） | Must |
...
| FR-045 | マトリクス UI の日付行は **1 か月単位** でページング表示する（**翌月のみ**。前月へのページングは v0.1.0 では提供しない） | Must |
...
- 列ヘッダー（デスク名）を固定
- 日付行は **1 か月単位** でページング（**翌月**ボタンのみ。当月より前の月は表示しない）。日付方向は縦スクロール
